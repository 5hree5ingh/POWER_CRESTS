...
import { Factory, ArrowRight, Box, Cpu, Hexagon, Cog, Blend } from 'lucide-react';
import AnimatedSection from '../components/AnimatedSection';
import StatCounter from '../components/StatCounter';
import { machinery } from '../data/products';

const iconMap = { Factory, Box, Cpu, Hexagon, Cog, Blend };
    ...
    <>
      ...
      <section className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-margin py-10 sm:py-20">
        <AnimatedSection className="mb-8 sm:mb-12">
          <h2 className="font-semibold text-[22px] sm:text-headline-lg text-on-surface mb-3 sm:mb-4">Our Facility</h2>
          <p className="text-on-surface-variant text-[14px] sm:text-[16px] max-w-xl">
            Located in Lakshar, Haridwar — our facility is designed for maximum production efficiency near one of India's key industrial corridors.
          </p>
        </AnimatedSection>
            ...
            <AnimatedSection key={item.title} delay={i * 0.1}>
              <div className="group rounded-xl overflow-hidden border border-outline-variant">
                ...
                <div className="p-3 sm:p-4 bg-surface-container-lowest">
                  <h3 className="font-semibold text-[14px] sm:text-[16px] text-on-surface">{item.title}</h3>
                </div>
              </div>
            </AnimatedSection>
          ))}
      ...
      <section className="bg-surface-container-low py-10 sm:py-20 border-y border-outline-variant">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-margin">
          <AnimatedSection className="text-center mb-10 sm:mb-16">
            <span className="font-label-sm text-label-sm text-primary uppercase tracking-[0.2em] block mb-3">Equipment</span>
            <h2 className="font-semibold text-[24px] sm:text-headline-lg text-on-surface">Machinery & Equipment</h2>
          </AnimatedSection>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {machinery.map((machine, i) => {
              ...
              return (
                <AnimatedSection key={machine.name} delay={i * 0.04}>
                  <div className="bg-surface-container-lowest p-4 sm:p-5 rounded-xl border border-outline-variant hover:shadow-md hover:border-primary/20 transition-all duration-300 flex items-center gap-3 sm:gap-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary/10 rounded-xl flex items-center justify-center shrink-0">
                      <Icon size={20} className="text-primary" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-[13px] sm:text-[15px] text-on-surface leading-tight">{machine.name}</h4>
                    </div>
                    <div className="bg-primary text-on-primary font-mono font-bold text-[13px] sm:text-[14px] px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg shrink-0">
                      ×{machine.qty}
                    </div>
                  </div>
                </AnimatedSection>
      ...
      <section className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-margin py-10 sm:py-20">
        <AnimatedSection className="mb-8 sm:mb-12">
          <h2 className="font-semibold text-[22px] sm:text-headline-lg text-on-surface mb-4">Production Capabilities</h2>
        </AnimatedSection>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          {[
            ...
            <AnimatedSection key={item.title} delay={i * 0.1}>
              <div className="bg-surface-container-lowest p-6 sm:p-8 rounded-xl border border-outline-variant h-full">
        ...
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-margin text-center">
          <AnimatedSection>
            ...
            <p className="text-on-secondary-fixed-variant text-[14px] sm:text-[17px] mb-6 sm:mb-8 max-w-md mx-auto">
              Schedule a visit to our Lakshar, Haridwar facility and see our manufacturing process firsthand.
            </p>
            <Link
              to="/contact"
